...
from app.pipeline.meeting.chunker import MeetingChunk, chunk_meeting
from app.pipeline.meeting.diarizer import diarize, release_diarization_pipeline
...
def run_meeting_pipeline(
    ...
    speaker_turns = merge_transcript_with_diarization(whisper_segments, diarization_segments)
    ...
    for chunk in chunks:
        ...
        for screen_seg in screen_segments:
            overlap = chunk.start < screen_seg.end and chunk.end > screen_seg.start
            if overlap:
                visual_emb = embed_image(screen_seg.frame_path)
                chunk.chunk_type = "screen_share"
                break

        result_chunks.append(MeetingChunkWithEmbedding(chunk=chunk, visual_embedding=visual_emb))
